...
if [ "$MODE" = "download" ]; then
	if [ ! -e "$DOWNLOAD_PATH" ]; then
		echo Fetching "http://fast.dpdk.org/rel/dpdk-${DPDK_VER}.tar.xz"
		curl http://fast.dpdk.org/rel/dpdk-${DPDK_VER}.tar.xz -o "${DOWNLOAD_PATH}"
	fi
	if [ ! -d "${DPDK_RESULT}" ]; then
		mkdir -p ${DPDK_RESULT}
	fi
	tar Jxvf "${DOWNLOAD_PATH}" -C "${DPDK_RESULT}" --strip-components=1
else
