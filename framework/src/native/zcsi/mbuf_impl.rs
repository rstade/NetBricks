use crate::native::zcsi::rte_mbuf_api::{PKT_TX_IP_CKSUM, PKT_TX_IPV4, PKT_TX_TCP_CKSUM, rte_mbuf};
use std::fmt;
use std::ptr;
...
impl MBuf {
    ...
    pub unsafe fn metadata_as<T: Sized>(mbuf: *const MBuf, slot: usize) -> *const T {
        unsafe { (mbuf.offset(1) as *const usize).offset(slot as isize) as *const T }
    }

    #[inline]
    pub unsafe fn mut_metadata_as<T: Sized>(mbuf: *mut MBuf, slot: usize) -> *mut T {
        unsafe { (mbuf.offset(1) as *mut usize).offset(slot as isize) as *mut T }
    }
